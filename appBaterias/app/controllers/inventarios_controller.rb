class InventariosController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @inventario.update(inventario_params)
        format.html { redirect_to @inventario, notice: 'Articulo Actualizado.' }
  ...
  def destroy
    @inventario.destroy
    respond_to do |format|
      format.html { redirect_to inventarios_url, notice: 'Articulo fue eliminado.' }
  ...
  private
    ...
    def inventario_params
      params.require(:inventario).permit(:codigoProducto, :codigoProveedor, :descripcion, :cantidad, :precioCosto, :utilidad, :impuesto, :precioVenta1, :precioVenta2)
    end
end
